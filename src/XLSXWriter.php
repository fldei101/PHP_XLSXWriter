...
class XLSXWriter
{
    ...
    protected function writeCell(BuffererWriter &$file, $row_number, $column_number, $value, $num_format_type, $cell_style_idx)
    {
        ...
        } elseif ($num_format_type == 'n_auto') { //auto-detect unknown column types
            if (!is_string($value) || $value == '0' || (ctype_digit($value)) || preg_match("/^\-?(0|[1-9][0-9]*)(\.[0-9]+)?$/", $value)) {
                $file->write('<c r="' . $cell_name . '" s="' . $cell_style_idx . '" t="n"><v>' . self::xmlspecialchars($value) . '</v></c>'); //int,float,currency
    ...
    public static function convertDateTime($date_input) //thanks to Excel::Writer::XLSX::Worksheet.pm (perl)
    {
        ...
        if (preg_match("/(\d+):(\d{2}):(\d{2})/", $date_time, $matches)) {
            ...
            $seconds = (floatval($hour) * 60 * 60 + floatval($min) * 60 + floatval($sec)) / (24 * 60 * 60);
        }
        ...
        if ($year != 0 || $month != 0 || $day != 0) {
            ...
            if ($day < 1       || $day > $mdays[intval($month) - 1]) {
                return 0;
            }
        }
